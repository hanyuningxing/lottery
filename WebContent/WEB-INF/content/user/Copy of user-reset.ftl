...
<!--	<link href="<@c.url value= "/pages/css/center_main.css"/>" rel="stylesheet" type="text/css" /> -->
  ...
  //短信倒计时时间
	...
	function msgVal(){
		...
		var url = window.BASESITE + '/user/user!msgValid.action?msgCode='+code+'&emailValForm.phoneNo='+phone;
		$.ajax({
		type : 'GET',
		cache : false,
		dataType : 'json',
  		...
  		<div class="cyy_zcnr">
				      ...
				      <font>邮箱：</font>
				       <#if user.info??&&user.info.email??&&user.info.email!="">
				         <div>
				     	 ...
				     	 <input id="emailInput" type="hidden" name="regForm.email"  value="${user.info.email!}">
				      <#else>
				     	<div><input class=text_inp type=text id="emailInput" name="regForm.email" <#if user.validatedEmail??&&user.validatedEmail=1>disabled="disabled"</#if> value="${user.email!}"></div>	(注:为了校验安全,补充完后不给修改)		 
				      </#if>
			...
			<div class="list_div" id="phone_list_div">
				      ...
				      <#if user.info??&&user.info.mobile??&&user.info.mobile!="">
				         <div>
				      	 <input class=text_inp type=text disabled="disabled"  value="${user.info.mobile?substring(0,3)}****${user.info.mobile?substring(7)}"></div>(注:为了校验安全,手机号码补充完后不给修改)
			...
			<#if user.info??&&user.info.mobile??&&user.info.mobile!="" && (!user.validatedPhoneNo??||!user.validatedPhoneNo)>
