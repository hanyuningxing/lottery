...
$(document).ready(function(){
	...
	$("#login_form").submit(function(){
        ...
        if(check_form()){
            ...
            $.ajax({
                type: "post",
                url : "<@c.url value= "/user/user!login.action?type=quick_login"/>",
                dataType:'json',
                async:false,
                ...
                success:function(json){               	
					if(json.success == true){
						...
						parent.$SSO.forum_login(json);
						 try{
						 	var sliUrl = json.sliUrl;
						 	if(sliUrl!=undefined && sliUrl!=null){
								$.getJSON(sliUrl, {format: "json"}); //, function(data) {}
							}else{
							}   
							
						 	if(parent != null){
							 	if(parent.$SSO != null && parent.$SSO.loginSuccessCall != null){
							 		parent.$SSO.loginSuccessCall();//刷新页头登录信息
							 	}
							 	if(parent._tipsHandle != null){
							 		parent._tipsHandle.close();
							 	}
						 	}else{
