<#if playType??>
    <#assign lotteryName=playType.text!"" />
</#if>
<head>
	<title>胜负足彩开奖公告、号码详情_最新开奖_历史开奖 - ${webapp.webName}安全购彩平台</title>
...
bordercolor="#dcdcdc" cellspacing="0" cellpadding="1" width="938"  
  border="1" align="center">
        ...
        <#if matchArr??>
	          ...
	          <td class="redboldchar" >${data.result!}</td>
	        </tr>
		  </#list>
		</#if>
      </table>
  ...
  border="1" align="center">
        <tr class="center_trgray font14char boldchar" align="center"height="30"  >
         ...
         <#if periodData??>
	            <tr class="center_trw" align="center" >
		          <td height="28">一等奖</td>
		          <td > <#if periodData.firstWinUnits_14??>${periodData.firstWinUnits_14}<#else>-</#if> </td>
		          <td class="redc" ><#if periodData.firstPrize_14??>${periodData.firstPrize_14?string('###,###')}<#else>-</#if>元 </td>
		        </tr>
		        <tr class="center_trw" align="center" >
		          <td height="28">二等奖</td>
		          <td ><#if periodData.secondWinUnits_14??>${periodData.secondWinUnits_14}<#else>-</#if> </td>
		          <td class="redc" ><#if periodData.secondPrize_14??>${periodData.secondPrize_14?string('###,###')}<#else>-</#if>元 </td>
		        </tr>
		        <tr class="center_trw" align="center" >
		          <td height="28">任选9场</td>
		          <td > <#if periodData.firstWinUnits??>${periodData.firstWinUnits}<#else>-</#if></td>
		          <td class="redc" ><#if periodData.firstPrize??>${periodData.firstPrize?string('###,###')}<#else>-</#if>元 </td>
		        </tr>
		 </#if>
      </table>
      <table style="background:#fff; border-collapse:collapse; border:1px solid #E3E3E3;"  
bordercolor="#dcdcdc" cellspacing="0" cellpadding="1" width="938"  
  border="1" align="center">
        ...
        <tr class="center_trw" align="center" >
          <td height="35" class="kj_buybg"><a href="javascript:void(0);" class="graychar">${lotteryName}数据图表</a> | <a href="${base}/info/rule/rule.action?lottery=SFZC&type=0" class="graychar">${lotteryName}玩法介绍</a> | <a href="${base}/info/rule/rule.action?lottery=SFZC&type=0" class="graychar">${lotteryName}中奖细则</a></td>
        </tr>
        <tr class="center_trw" align="center" >
