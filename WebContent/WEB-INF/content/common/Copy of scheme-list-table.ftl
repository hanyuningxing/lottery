	...
	<tbody>
      <#if pagination.result?? && pagination.result?size gt 0>
		...
		<tr class="row${(data_index+1)%2}" ${styleStr}  onmouseout="this.className=''" onmouseover="this.className='trhover'">
            ...
            <td  align="center"><a href="<@c.url value="/${data.lotteryType.key}/scheme!show.action?schemeNumber=${data.schemeNumber}" />" <#if data.keepTop!false>style="color:red"</#if>>${data.schemeNumber}</a></td>
