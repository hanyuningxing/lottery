<@override name="title">
	<title>中国体育彩票超级大乐透网上代购合买平台-大乐透投注-网上购买大乐透 - ${webapp.webName}</title>
</@override>
	...
	<table width="98%" border="0" cellspacing="0" cellpadding="0" class="hmTable">
		<thead>
			<tr align="center">
			  	...
			  	<td width="45%" nowrap="nowrap">开奖号码</a></td>
			  	...
			  	<td width="20%" nowrap="nowrap">开奖时间</td>
			</tr>
		</thead>
		<tbody>
					...
					<tr class="row${(data_index+1)%2}"  onmouseout="this.className=''" onmouseover="this.className='trhover'">
		          		...
		          		<td align="center">        
		          			<#if data.periodData.result??>
		                		...
		                		<ul class="kjball">
		                			<#list 0..4 as red_index>
		                 				<li class="redballsingle">${resultArr[red_index]!}</li>
		                			</#list> 
		                			<#list 5..6 as blue_index>
		                 				<li class="blueballsingle">${resultArr[blue_index]!}</li>
		                			</#list> 
		                		</ul>
		              		</#if>
		          		</td>
