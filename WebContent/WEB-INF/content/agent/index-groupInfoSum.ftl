...
<script>
	   function seeUser(userId,userName){
		    $floater({
				width : 250,
				height : 250,	
				src : window.BASESITE + '/agent/index!seeAgentRebate.action?userId='+userId,
				title : userName+'的返点',
				fix : 'true'
			});
	   }
          ...
          <tr>                                                                                                            
           ...
           <#if pagination??&& (pagination.result![])?size gt 0>
		    	...
		    	<tr class="${trColor}" onmouseover="this.className='trlbro'" onmouseout="this.className='${trColor}'">
			      ...
			      <td><a href="#" onclick="seeUser('${data.userId!}','${data.userName!}');">查看返点</a> | <a href="${base}/agent/index!findAgentGroupInfo.action?userId=${data.userId!}">查看明细</a> | <a href="${base}/agent/user!fundList.action?userId=${data.userId!}" target="_blank">资金明细</a></td>
			     </tr>
		    	</#list>
		    <#else> 
		    <tr>
