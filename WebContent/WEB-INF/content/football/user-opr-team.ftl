...
$(document).ready(function(){
	$("#opr_form").submit(function(){
        $("#submit").attr('disabled',true);
            ...
            var sclass = $("select[name=sclass]").val();
            $.ajax({
                type: "post",
                url : "<@c.url value= "/football/user!oprTeam.action"/>",
                dataType:'json',
                async:false,
                data: {
                      teamId:teamId,
...
<body>
	<div class="login_title">
	    <ul class="login_table">  
	        <input name="teamId" id="teamId"  type="hidden"  value="<#if teamId??>${teamId}</#if>"/>
		    <table width="100%" border="0" cellspacing="0" cellpadding="0">
		        <form id="opr_form" action="" method="post" autocomplete="off">
		            ...
		            <tr>
		                <td width="25%">联赛级别选择：</td>
		                <td width="75%" align="left">
					          <select name="sclass">
					             <#if sclassList??>
						           <#list sclassList as sclass>
						              <option value="${sclass.id}" <#if sclass??&&sclass.id??&&sclass.id ==sclassId>selected</#if>>${sclass.name_J}</option>
						           </#list>
						         </#if>
						      </select>
		                </td>
