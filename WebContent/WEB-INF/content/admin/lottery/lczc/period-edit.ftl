...
<head>
	...
	<script type='text/javascript'>
		...
		function addRow(obj){
			...
			trEl.grab(new Element('td', {
				'html' :'<input name="matchs['+obj.index+'].halfHomeScore" value="" style="width:40px;">'
			}));
			trEl.grab(new Element('td', {
				'html' :'<input name="matchs['+obj.index+'].homeScore" value="" style="width:40px;">'
			}));
			trEl.grab(new Element('td', {
				'html' :'<input name="matchs['+obj.index+'].halfGuestScore" value="" style="width:40px;">'
			}));
			trEl.grab(new Element('td', {
	...
	<form action="<@c.url value='/admin/lottery/${lotteryType.key}/period!updateMatch.action' />" method="post"   id="matchForm">
		...
		<table id="matchTab" width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#DCDCDC">
		  <tr class="twonavlblue">
		    ...
		    <td colspan="12"><input type="button" value="抓取赛果" id="lczcFetch"/></td>
		  </tr>
		  ...
		  <#if matchs??>
	  		<#list matchs as match>
			    ...
			    <td><input name="matchs[${match_index}].halfHomeScore" id="halfHomeScore_${match_index}" value="${match.halfHomeScore!}" style="width:40px;"> </td>
			    <td><input name="matchs[${match_index}].homeScore" id="homeScore_${match_index}" value="${match.homeScore!}" style="width:40px;"> </td>
			    <td><input name="matchs[${match_index}].halfGuestScore" id="halfGuestScore_${match_index}" value="${match.halfGuestScore!}" style="width:40px;"></td> 
...
<div>
	<form action="<@c.url value='/admin/lottery/${lotteryType.key}/period!updatePeriodData.action' />" method="post">
		...
		<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#DCDCDC">  
          <tr>
		    <td align="left" colspan="4"><label>六场足彩开奖数据</label></td>
		  </tr>
		  <tr class="trw">
          ...
          <tr class="trw">
            ...
            <td align="left">
    	       <input type="text" name="periodData.prizePool" id="periodData.prizePool" value="${periodData.prizePool!}" />
            </td>
          </tr>
          <tr class="trlblue">
