		...
		<thead>
		  <tr align="center" class="dctro">
	          ...
	          <#list itemArr as item>
	        	<td width="<#if isSingle>37<#else>40</#if>"><label for="column_${item.ordinal()}">${item.text}</label></td>
	          </#list>
	          <td width="48">比分</td>
	          <td width="48">分析</td>	        
	    ...
	    <tbody id="matchTbody">
	      ...
	      <#if matchMap??>
	  		<#list matchMap.entrySet() as entry>
		        ...
		        <td colspan="${(itemArr?size)+col}" height="22" height="22" ><img height="15" width="40" align="absmiddle" src="<@c.url value="/pages/images/btyc.gif" />" onclick="dayFilter(this);" _di="${entry_index}" style="cursor:pointer;" /> <span class="boldchar">${entry.getKey()}（10：00 -- 次日 10：00）两队之间数字为让球数 “-”负号表示主队让客队</span></td>
			  </tr>
			  <#list entry.getValue() as match>
				  ...
				  <tr id="tr_${match.lineId}" _lid="${match.lineId}" class="${trClass}" align="center" onmouseover="this.className='trhover'" onmouseout="this.className='${trClass}'" _m="1" _d="${entry_index}" <#if games?? && match.gameName?? && match.gameName?trim !=''>_g="${games.indexOf(match.gameName)}"</#if> _h="${match.handicap!0}" _e="<#if saleEnd>1<#else>0</#if>" <#if saleEnd>style="display:none"</#if>>
			       ...
			       <!--<td><#if endSaleTime??>${endSaleTime?string('HH:mm')}</#if></td>-->
			        ...
			        <#list itemArr as item>
			        <td class="align_l" <#if match.cancel>title="取消"</#if> <#if !saleEnd>id="td_${match.lineId}_${item.ordinal()}" onclick="clickItem(${match.lineId},${item.ordinal()},event);" style="cursor: pointer;"</#if> >
