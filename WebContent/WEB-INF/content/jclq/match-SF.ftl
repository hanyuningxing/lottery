			...
			<#if matchMap??>
				<#list matchMap.entrySet() as entry>
					...
					<#list entry.getValue() as match>
						<#assign open_SF=match.isOpen(SF,passMode)/>
						<#if match_index%2==0>
					    ...
					    <tr class="${trClass}" align="center" id="tr_${match.matchKey}" _lid="${match.matchKey}" _m="1" _d="${entry_index}" <#if games?? && match.gameName?? && match.gameName?trim !=''>_g="${games.indexOf(match.gameName)}"</#if> _h="${match.handicap!0}" _e="<#if saleEnd>1<#else>0</#if>">
						    ...
						    <#if open_SF>
					            <#list itemArr as item>
					            	<td width="75" class="tdyelnor" <#if match.cancel>title="取消"</#if> <#if !saleEnd>id="SF_td_${match.matchKey}_${item.ordinal()}" onclick="clickItem(${match_index},'${match.matchKey}',${item.ordinal()},event,'SF');"</#if>>
					            ...
					            <td width="32" class="tdyelnor" <#if !saleEnd>onclick="rowBatch(${match_index},'${match.matchKey}',0,event,'SF')"</#if>><#if !saleEnd><input id="SF_bao_0_${match.matchKey}" type="checkbox" onclick="rowBatch(${match_index},'${match.matchKey}',0,event,'SF')" align="absmiddle" autocomplete="off" /><#else>&nbsp;</#if></td>
							<#else>
