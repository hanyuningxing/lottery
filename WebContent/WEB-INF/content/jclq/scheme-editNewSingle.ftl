...
<div class="border04 marginb20">
	...
	<div class="sssx01" id="stepOneOper">
	  	<div style="float:left;"><a class="ssxsan" href="javascript:;" onclick="javascript:$$('sssxxzdiv').style.display='';"></a>&nbsp;&nbsp;隐藏 <strong><span id="c_h" class="rc1">0</span></strong> 场比赛&nbsp;&nbsp;<a href="javascript:;" onclick="matchFilterReset();return false;" class="blue_0091d4">恢复</a>&nbsp;&nbsp;
	  		<input id="s_hdp1" style="display:none;" onclick="matchFilter();" checked="checked" type="checkbox" autocomplete="off"/>让球(<span class="rc1">${handicapCount!}</span>)&nbsp;&nbsp;
	        <input id="s_hdp0" style="display:none;" onclick="matchFilter();" checked="checked" type="checkbox" autocomplete="off"/>非让球(<span class="rc1">${unHandicapCount!}</span>)
	    </div>
	...
	<#else>
		<div style="line-height:60px;text-align:center;height:60px;border:1px dashed #ccc;border-top:none;background:#F4F5F3;">暂无可投注的赛事.</div>
	</#if>	  
	</div>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#dae8f5" class="jjyhtab">
          <tr>
            ...
            <td bgcolor="#FFFFFF">
            	...
            	格式说明：<#if itemMap??><#list itemMap.get(playType?string) as item><#if item_index lt 3>${item.text}→${item.value}&nbsp;</#if><#if item_index==3>...</#if><input type="hidden" name="createForm.codes" _d="${item.value}" value="${item.value}"/></#list></#if><a href="javascript:;" id="update_single">修改</a> <br/>
            ...
            <td bgcolor="#FFFFFF">
            	<div id="common_logined" style="display:none">
            		<span class="gc0" id="common_login_name"></span>
            			<strong><span class="rc1" id="common_remainMoney"></span></strong> 元 [ <a id="confirm_tz_payPer_a" href="${base}/user/fund!payPer.action" target="_blank">帐户充值</a> ]
            	</div>
            	
            		<#if loginUser??>
	              		<div id="user_logined">
	              			<strong><span class="rc1" id="common_remainMoney">${loginUser.remainMoney?string.currency}</span></strong> 元 [ <a id="confirm_tz_payPer_a" href="${base}/user/fund!payPer.action" target="_blank">帐户充值</a> ]		              		
		              	</div>	       
		        	<#else>
          ...
          <tr>
            <td height="50" colspan="2" bgcolor="#FFFFFF" style="padding-left:117px;"><span id="span_createForm_submit_"><a id="formSubmit" onclick="submitCreateForm();return false;" href="#" class="btbuy"></a></span><span style="display: none;" id="span_createForm_waiting_">正在提交,请稍等...</span></td>
          </tr>
        </table>
      </div>
    </div>      
...
<td align="center">上传字符</td>
						<#if itemMap??>
            				...
            				<#list itemMap.get(playType?string) as item> 
	            					...
	            					<td style="text-align: center;">         					
	            						<input id="TempCode${index}" type="text" style="text-align: center;" autocomplete="off"  value="${item.value}" size="2" maxlength="1"> 
	            					</td> 
...
<#else>
	...
	<div style="padding-left:10px;">
		<span id="shuoming0">
		<#if playType=="RFSF" || playType=="SF">
			<br>
			选择5场比赛5串1如：30303 或 3,0,3,0,3
			<br>
			选择5场比赛4串1如：303*0 或 3,0,3,*,0
		<#elseif playType="SFC">
			<br>
			选择3场比赛3串1如：011305 或 01,13,05
			<br>
			选择3场比赛3串1如：01**05 或 01,**,05
			<br>
		<#elseif playType="DXF">
			<br>
			选择5场比赛5串1如：10101 或 1,0,1,0,1
			<br>
			选择5场比赛4串1如：101*0 或 1,0,1,*,0
			<br>	
		</#if>	
			<br>
			</span>
