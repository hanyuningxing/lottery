...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="border:1px solid #E0E1E2;" class="cb">
	...
	<#list pagination.result![] as data>
	    <tr align="center" valign="middle" class="<#if data_index%2==0>bgbai<#else>bghui</#if>" >
	      ...
	      <td align="left">
	      	<#list data.matchItemList as matchItem>
  				...
  				<#if match.cancel>
  					<#assign r=-1 />
  				<#elseif match.ended>
  					<#assign rsOp = match.getResult(matchItem.playType)!'' />
  					<#if rsOp != ''>
	      		...
	      		${match.matchKeyText}<#if matchItem.playType??>[${matchItem.playType.text}]<#if matchItem.playType=='RQSPF'><#if match.handicap??>(${match.handicap})</#if></#if></#if>
	      		<#list matchItem.playType.allItems as item>
	      ...
	      <td><#if data.printed>已出票<#else>未出票</#if></td>
