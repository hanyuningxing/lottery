	    ...
	    <td>
	    	<#if matchItemMap?exists>
	    		...
	    		<#list matchItems as matchItem>
	    			<#if matchItem.playType==rqspf>(${match.handicap!0})</#if>${matchItem.buildSelectedItemStr()}<br/>
	    		</#list>
	    	</#if>
	    </td>
	    <#assign minMaxSp = minMaxSpOfMatchMap[matchkey]>
	    <td>${minMaxSp[0]?string('0.00')}</td>
	    <td><#if minMaxSp[1].get(rqspf)?exists><#if minMaxSp[1].get(rqspf)==0>-- --<#else>${minMaxSp[1].get(rqspf)?string('0.00')}</#if></#if></td>
	    <td><#if minMaxSp[1].get(spf)?exists><#if minMaxSp[1].get(spf)==0>-- --<#else>${minMaxSp[1].get(spf)?string('0.00')}</#if></#if></td>
	    <td><#if minMaxSp[1].get(bqq)?exists><#if minMaxSp[1].get(bqq)==0>-- --<#else>${minMaxSp[1].get(bqq)?string('0.00')}</#if></#if></td>
	    <td><#if minMaxSp[1].get(jqs)?exists>${minMaxSp[1].get(jqs)?string('0.00')}</#if></td>
	    <td><#if minMaxSp[1].get(bf)?exists><#if minMaxSp[1].get(bf)==0>-- --<#else>${minMaxSp[1].get(bf)?string('0.00')}</#if></#if></td>
