	 ...
	 <div align="center" style="text-align:center;padding:6px 0px 6px 0px;font-family:Verdana,Arial;word-spacing:5px;font-size:12px;background:#F9F9F9;">
		<#if totalPages <= showNum+2>
			<div class="cb pagelist" align="center">
				<a class="pagenum" href="<@c.url value='${pageUrl+"pagination.pageNo="+1}' />" onclick="return fn_turnPage(this);">首页</a>	
				<#if pageNo!=1>
					<a class="pagenum" href="<@c.url value='${pageUrl+"pagination.pageNo="}${pageNo-1}' />" onclick="return fn_turnPage(this);">上一页</a>											
				<#else>
					<a class="pagenum" href="<@c.url value='${pageUrl+"pagination.pageNo="+1}' />" onclick="return fn_turnPage(this);">上一页</a>					
				</#if>
			<#list 1..totalPages as index>
				<#if index!=pageNo>
					<a class="pagenum" href="<@c.url value='${pageUrl+"pagination.pageNo="+index}' />" onclick="return fn_turnPage(this);"><#if index==pageNo>${index}<#else>${index}</#if></a>									
				<#else>
					<a class="nowpage" href="<@c.url value='${pageUrl+"pagination.pageNo="+index}' />" onclick="return fn_turnPage(this);"><#if index==pageNo>${index}<#else>${index}</#if></a>					
				</#if>
			</#list>
				<#if pageNo!=totalPages>
					<a class="pagenum" href="<@c.url value='${pageUrl+"pagination.pageNo="}${pageNo+1}' />" onclick="return fn_turnPage(this);">下一页</a>															
				<#else>
					<a class="pagenum" href="<@c.url value='${pageUrl+"pagination.pageNo="}${totalPages}' />" onclick="return fn_turnPage(this);">下一页</a>						
				</#if>
				<a class="pagenum" href="<@c.url value='${pageUrl+"pagination.pageNo="}${totalPages}' />" onclick="return fn_turnPage(this);">尾页</a>	
			</div>
		<#else>
			<#-- ************* -->
