...
compute.view={
	    ...
	    getTerm:function()
	   {
	        ...
	        var canCapacityChaseIssue=document.getElementById("startChasePeriod");
	        var mzzs_select=document.getElementById("mzzs_select");
	        var betType_prizeselect = document.getElementById("betType_prize");
	        var  prize = parseInt(betType_prizeselect.options[betType_prizeselect.selectedIndex].value);
	        var createForm = getCreateForm();
	    	var salesMode = createForm.elements['createForm.salesMode'].value;
	    	mzzs_select.options.length = 0;
		    var chaseNumUrl= window.BASESITE+"/"+lotteryType_key+"/scheme!canCapacityChaseIssue.action?createForm.salesMode="+salesMode+"&betType="+betType+"&createForm.content="+content;
				$.ajax({ 
						type: 'GET',
						cache : false,
						url: chaseNumUrl,
						success : function(data) {
							var kenoPeriodList = toJsonObject(data.kenoPeriodList);
							if(data.bingoCountList!=null) {
								var bingoCount = toJsonObject(data.bingoCountList);
								for(var i=bingoCount.length-1;i>=0;i--){
					        		mzzs_select.options.add(new Option(bingoCount[i],parseInt(bingoCount[i])*prize));
					        	}
							} else {
				        		mzzs_select.options.add(new Option(1,prize));
							}
							
					        for(var i=0;i<kenoPeriodList.length;i++){
							   	if(i<30) {
							   		canCapacityChaseIssue.options.add(new Option(kenoPeriodList[i].periodNumber,kenoPeriodList[i].id));
							    }
							    compute.model.termList.push(kenoPeriodList[i].periodNumber);
					        }
						}
				});
	   },
