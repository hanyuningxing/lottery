$(function() {
	...
	var item_arr = [ 'nonGoal', 'oneGoal', 'twoGoal', 'threeGoal' ];
	var item_text_arr = ['0','1','2','3+'];
	var selectedArr = [ 0, 0, 0, 0, 0, 0, 0, 0 ];// 注：在页面定义场次数目，这里在根据场次数目生成数组，可灵活适用于足彩其他几个玩法
	...
	window.rowSelect = function(matchIndex) {
		var createForm = getCreateForm();
	...
	window.buildConfirmData = function (){		
		...
		var itemsHtml='<table width="730" border="0" cellspacing="0" cellpadding="0" class="vtable">';
			itemsHtml+='<tr class="v01tr"><td>场次 </td><td>主队/客队 </td><td colspan="2">您的选择</td></tr>';
		for(var i=0;i<selectedArr.length;i++){
			var selectedItemStr = '';
			...
			if(i%2==0){
				...
				itemHtmlOfMatch +='<td>'+ $$('home_'+ index).innerHTML +'</td>';
			}else{
				itemHtmlOfMatch +='<td>'+ $$('guest_'+ index).innerHTML+'</td>';
			}
