...
$.fn.tbtips = function(a) {
    ...
    var a = $.extend(b, a);
    getTip = function() {
        var c = '<div id="tbTips" class="tb-tips"><div class="hd"></div><div class="bd"><h4 class="tips-q"></h4><blockquote class="tips-a"></blockquote></div><div class="ft"></div></div>';
        return c
    };
    $("body").append(getTip());
    $(this).each(function() {
        ...
        function() {
            stopTimer();
            h.removeAttr("style").hide()
        });
        setTimer = function() {
            i.showTipTimer = setInterval("showTip()", b.delay)
        };
        stopTimer = function() {
            clearInterval(i.showTipTimer)
        };
        setTip = function(o, n) {
            var l = h.height(),
            m = n + "px",
...
function changeClass(obj) {
    if (!$(obj).hasClass('active')) {
        $('#searchTabs').find('a').removeClass('active');
        $(obj).addClass('active');
    }
    if ($('#searchTabs').find('.active').text() == "查宝贝返利") {
        $('#keywords').val("亲，在这里输入宝贝名称或复制宝贝网址");
        keytext = "亲，在这里输入宝贝名称或复制宝贝网址{|}也可以复制宝贝标题（马克华菲 新款深紫色格纹夹克）到这里{|}点击开始淘，查到返利啦，赶快去购买吧 ^_^";
        keyarr = keytext.split('{|}');
        $("body").data("flag", "item");
    } else {
        $('#keywords').val("输入你想查找的店铺名称或掌柜名字");
        keytext = "输入你想查找的店铺名称或掌柜名字{|}输入你想查找的店铺名称或掌柜名字{|}输入你想查找的店铺名称或掌柜名字";
        keyarr = keytext.split('{|}');
        $("body").data("flag", "shop");
    }
}
function search_blur() {
    if ($("body").data("flag") == "item") {
        if ($("#keywords").val() == '') $("#keywords").val("亲，在这里输入宝贝名称或复制宝贝网址").addClass('placeholder');
    } else {
        if ($("#keywords").val() == '') $("#keywords").val("输入你想查找的店铺名称或掌柜名字").addClass('placeholder');
    }
}
function enterPanduan(event) {
    if (event.keyCode == 13) {
        search();
    }
}
function search_focus() {
    ...
    if (('{|}' + keytext + '{|}').indexOf('{|}' + $('#keywords').val() + '{|}') != -1) {
        $('#keywords').val('').removeClass('placeholder');
    }
    if ($("body").data("flag") == "item") {
        if ($("#keywords").val() == '亲，在这里输入宝贝名称或复制宝贝网址') $("#keywords").attr("value", "").removeClass('placeholder');
    } else {
        if ($("#keywords").val() == '输入你想查找的店铺名称或掌柜名字') $("#keywords").attr("value", "").removeClass('placeholder');
    }
}
function search(prefix) {
    var keywords = $("#keywords").val();
    if (keywords == "" || keywords == '亲，在这里输入宝贝名称或复制宝贝网址' || keywords == '也可以复制宝贝标题（马克华菲 新款深紫色格纹夹克）到这里' || keywords == '点击开始淘，查到返利啦，赶快去购买吧 ^_^' || keywords == '输入你想查找的店铺名称或掌柜名字') {
        alert("请输入关键字");
        return;
    }
    var newUrl = "";
    if ((APP.indexOf("index.php") == -1) && (location.href.indexOf("?") == -1)) {
        keywords = encodeURIComponent(encodeURIComponent(encodeURIComponent(keywords)));
    } else {
        keywords = encodeURIComponent(encodeURIComponent(keywords));
    }
    if ($("body").data("flag") == 'item') {
        newUrl = APP + "/search/searchTaobaoItems/keywords/" + keywords;
        tail = "productsearch";
    } else {
        newUrl = APP + "/search/searchTaobaoShops/keywords/" + keywords;
        tail = "storesearch";
    }
    Ftrack(prefix, tail);
...
function searchStop(keywords) {
    ...
    window.location.href = APP + "/search/searchTaobaoShops/keywords/" + keywords;
}
...
function jumpTaobaoSp(commission, spurl) {
    url = APP + "/shop/jumpTbShop/commission/" + commission + "/tburl/" + spurl;
    var subWindow = window.open(url, 'newwindow', 'height=800, width=800, top=100, left=200, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, status=no');
}
function show_child(id) {
    if ($("#child" + id).css("display") == "block") {
        return;
    }
    $("span", $(".sousuodplb_taodianlb_content")).each(function() {
        $(this).css("display", "none");
    });
...
function jump(id) {
    ...
    if ((APP.indexOf("index.php") == -1) && (location.href.indexOf("?") == -1)) {
        area = encodeURIComponent(encodeURIComponent(encodeURIComponent(area)));
    } else {
        area = encodeURIComponent(encodeURIComponent(area));
    }
    window.location.href = pre_Link + '/area/' + area;
}
function show_sx(id) {
    $("#" + id).show();
}
...
function open_tab(id) {
    var conid = id.replace("t", "tc");
    if ($("#" + conid).attr("class") == "dise") {
        return;
    }
    $("img[id^='t']").attr("src", PUBLIC + "/images/dingdanxg_06.jpg");
    $("#" + id).attr("src", PUBLIC + "/images/dingdanxg_03.jpg");
    $("li[id^='tc']").attr("class", "dise1");
    $("#" + conid).removeClass("dise1");
    $("#" + conid).addClass("dise");
}
function send_feadback() {
    var msg = $("#msg").val();
    var contact = $("#contact").val();
    $.ajax({
        type: "POST",
        url: APP + "/faq/add_feadback",
        data: "msg=" + msg + "&contact=" + contact,
        success: function(msg) {
            alert(msg);
            window.location.href = APP + "/faq/yjjy";
        }
    });
}
function addBookmark(title, url) {
    var alertMsg = "请使用Ctrl+D进行收藏添加";
    if (window.sidebar) {
...
function change_tab1(type) {
    var con_1 = document.getElementById('1');
    var con_2 = document.getElementById('2');
    if (type == 1) {
        con_1.className = 'kuang';
        con_2.className = '';
    } else if (type == 2) {
        con_1.className = '';
        con_2.className = 'kuang';
    }
}
function trim(s) {
    return s.replace(/^\s*/, "").replace(/\s*$/, "");
}
function showhidemyflwmenu() {
    var $myflw = $('#myflw');
    var $myflwmenu = $('#myflwmenu');
    if ($myflw.hasClass('flwindex_juzhong_left_span4')) {
        $myflw.attr('class', 'flwindex_juzhong_left_span2');
        $myflwmenu.show();
    } else {
        $myflw.attr('class', 'flwindex_juzhong_left_span4');
        $myflwmenu.hide();
    }
}
function open53kf() {
...
function suggest() {
    $("#keywords").autocomplete(APP + "/search/suggst/t/" + Math.random(), {
        delay: 3,
        max: 10,
        minChars: 1,
        matchSubset: 1,
        ...
        formatItem: function(item) {
            return "<div><span style=\"float:left;\">" + item.name + "</span><span style=\"float:right;\">共找到宝贝数量" + item.count + "个</span></div>";
        },
...
function tab_brands() {
    ...
    $spotlight.tabs({
        ...
        event: 'mousedelay'
    }).tabs('rotate', 5000).delegate('ul.ui-tabs-nav li a', 'mouseover',
    function() {
    ...
    $spotlight.hover(function() {
        $spotlight.tabs('rotate', 0);
    },
    function() {
        $spotlight.tabs('rotate', 5000);
    });
    $("#brand").tabs();
}
function Ftrack(base) {
    ...
    length = arguments.length; i < length; i++) {
        _gaq.push(["pageTracker._trackPageview", base + '_' + arguments[i]]);
    }
    return false;
}
function close_steps() {
    $.cookie('tb-steps', '1', {
        expires: 1,
        domain: '.51fanli.com',
        path: '/'
    });
    $('#tb-steps').slideUp('slow');
}
function recordurl(url, flag) {
    window.open(url);
}
jQuery.keyText = function(n) {
    if (keyflag) {
        $('#keywords').val(keyarr[n]);
        if (n + 1 >= keyarr.length) {
            n = 0;
        } else {
            n++;
        }
        ...
        $('#keywords').removeClass('placeholder');
    }
}
function showpro(id) {
    if (0 >= ($('#brand-' + id).find('ul li').length)) {
        $('#brand-' + id).load(APP + '/common/ajaxGetHotShop/id/' + id);
    }
}
var _bdhmProtocol = (("https:" == document.location.protocol) ? " https://": " http://");
document.write(unescape("%3Cscript src='" + _bdhmProtocol + "hm.baidu.com/h.js%3F545c20cb01a15219bfeb0d1f103f99c1' type='text/javascript'%3E%3C/script%3E"));
var _gaq = _gaq || [];
_gaq.push(['_setAccount', 'UA-903436-1']);
_gaq.push(['_addOrganic', 'baidu', 'word']);
_gaq.push(['_addOrganic', 'soso', 'w']);
_gaq.push(['_addOrganic', 'sogou', 'query']);
_gaq.push(['_addOrganic', 'youdao', 'q']);
_gaq.push(['_setDomainName', '.51fanli.com']);
_gaq.push(['_trackPageview']);
_gaq.push(['_trackPageLoadTime']);
_gaq.push(['pageTracker._setAccount', 'UA-903436-10']);
_gaq.push(['pageTracker._trackPageview']); (function() {
...
function ShowMessageBox(option) {
    ...
    if (navigator.userAgent.indexOf("Chrome") >= 1) {
        MessageBox_scrolltop = window.pageYOffset;
    }
    ...
    if (option.height) height = parseInt(option.height);
    if (option.width) width = parseInt(option.width);
    ...
    enabled = '<div id="messagebox_enabled" style="background-color: #000;width:100%;height:100%;position:absolute;z-index:99998;"></div>'enabledframe = '<iframe id="messagebox_enabledframe"  frameborder=0 scrolling=no style="position:absolute; visibility:inherit; top:0px; left:0px; width:100%;height:100%;z-index:99997; "></iframe>';
    ...
    function CreateContainer(option) {
        var html;
        html = '<div id="messagebox_win" style="position:absolute;z-index:99999;"><div style=" width:100%;height:100%;"><div style="height:3px; overflow:hidden;background:#333;filter:alpha(opacity=40);-moz-opacity:0.4;opacity:0.4;"></div><div id="msg_left" style="width:3px;overflow:hidden;background:#333;filter:alpha(opacity=40);-moz-opacity:0.4;opacity:0.4; float:left;"></div><div id="msg_body" style="float:left;"><div id="messagebox_from" style=" background:#fff; font-size:12px;"></div></div><div id="msg_right" style=" width:3px;overflow:hidden;background:#333;filter:alpha(opacity=40);-moz-opacity:0.4;opacity:0.4; float:left;"></div><div style="height:3px; clear:both; overflow:hidden;background:#333;filter:alpha(opacity=40);-moz-opacity:0.4;opacity:0.4;"></div></div></div>';
        if (option.parent) {
    ...
    function GetOpacity(css) {
        if (window.navigator.userAgent.indexOf('MSIE') >= 1) {
            css.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=40)';
        } else {
            css.opacity = '0.4';
        }
    }
    function SetStyle(option) {
        ...
        region = GetDocumentRegion();
        css = {
            width: MessageBox_width + 'px',
            height: MessageBox_height + 'px',
            left: ((region.width - MessageBox_width) / 2) + 'px',
            top: ((region.height - MessageBox_height) / 2) + 'px'
        }
        if (region.height < MessageBox_height) {
            css.top = 10 + 'px';
        } else {
            css.top = ((region.height - MessageBox_height) / 2 + document.documentElement.scrollTop) + 'px';
            if (navigator.userAgent.indexOf("Chrome") >= 1) {
                css.top = ((region.height - MessageBox_height) / 2 + window.pageYOffset) + 'px';
            }
        }
        $('#messagebox_win').css(css);
        css.top = '0px';
        css.left = '0px';
        css.width = (MessageBox_width - 6) + 'px';
        css.height = (MessageBox_height - 6) + 'px';
        $('#msg_body').css(css);
        css.height = (MessageBox_height - 6) + 'px';
        css.width = '3px';
        $('#msg_left').css(css);
    ...
    function handleMouseDown(e) {
        ...
        cy = (parseInt(MessageBox_win.css('top')) | 0) MessageBox_Moving = true;
