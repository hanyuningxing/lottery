...
@Service("xjel11to5MissDataControllerServiceImpl")
public class XjEl11to5MissDataControllerServiceImpl extends
		MissDataControllerServiceImpl<XjEl11to5MissDataInfo, XjEl11to5IssueData> {
	@Autowired
	...
	public void createMissFiles() {
		...
		List<XjEl11to5MissDataInfo> list = missDataEntityManager.getLastMissDatas(500);
		...
		// 创建遗漏文件
		for (XjEl11to5MissDataInfo info : list) {
			JSONObject miss = new JSONObject();
			Map<String, Integer> map = info.getMissDataMap();// .getMissDataContent().toMap();
			for (int i = 1; i <= 11; i++) {
				String field = "num" + i;
				String field_1="num_one"+i;
				String field_2="num_two"+i;
				String field_3="num_three"+i;
				String field_g2="num_g_two"+i;
				String field_g3="num_g_three"+i;
				miss.put(i, map.get(field));
				num1miss.put(i,map.get(field_1));
				num2miss.put(i, map.get(field_2));
				num3miss.put(i, map.get(field_3));
				numG2miss.put(i, map.get(field_g2));
				numG3miss.put(i, map.get(field_g3));
			}
			nummiss = miss;
		...
		if(list.size()!=0&&list!=null){
			List<XjEl11to5MissDataInfo> list_static=new ArrayList<XjEl11to5MissDataInfo>();
			if(list.size()<=80){
				list_static=list;
			}else{
				list_static=list.subList(list.size()-81,list.size()-1);
			}
			...
			for(XjEl11to5MissDataInfo info_result:list_static){
				list_result.add(info_result.getResult());
			}
			for(String str:list_result){
				String[] strA=str.split(",");
				for(String str1:strA){
					map_static.put(Integer.valueOf(str1),map_static.get(Integer.valueOf(str1))+1);
				}
			}
			...
			createMaxMissFiles(nummiss, num1miss, num2miss, num3miss, numG2miss, numG3miss,numStatic);
		}
		WriteHTMLUtil.writeHtm("/js/analyse/" + getLottery().getKey() + "/", "el11to5data.js", "dataEl11to5="
				+ resultRoot.toString() + ";function getdata(){return dataGdEl11to5;}", "UTF-8");
	...
	private void createMaxMissFiles(JSONObject nummiss, JSONObject num1miss, JSONObject num2miss, JSONObject num3miss,
			JSONObject numG2miss, JSONObject numG3miss,JSONObject numStatic) {
		...
		WriteHTMLUtil.writeHtm("/js/analyse/" + getLottery().getKey() + "/", "xjel11to5_max_miss.js",
				"xjel11to5_max_miss=" + root + ";", "UTF-8");
	}
	...
	@Override
	public MissDataEntityManagerImpl<XjEl11to5MissDataInfo, XjEl11to5IssueData> getMissDataManager() {
	...
	public List<AnalyseFilter> getAnalyseFilters(NumberAnalyse ns) {
		...
		// 红球冷热分析项
		list.add(new AnalyseFilter("num", ns, 1, 11) {
	...
	public void updateGroupNumMiss(int periodCount) {
		...
		List<XjEl11to5IssueData> list = null;
		DetachedCriteria criteria = DetachedCriteria.forClass(XjEl11to5IssueData.class);
//		criteria.add(Restrictions.isNotNull("results"));
		...
		for (String contentKey : contentKeys) {
			...
			for (XjEl11to5IssueData data : list) {
...
//						map.put("bp", Long.valueOf(map.get("ep") + ""));
	 ...
	 */
		private JSONObject numStatic(JSONObject numStatic){
			JSONObject root=new JSONObject();
			Map<String, Integer> missDataMap = MapSortUtil.sortByIntegerValueDesc(numStatic);
	...
	private void createMaxMissFiles(JSONObject miss1,JSONObject miss2,JSONObject miss3,JSONObject miss4,JSONObject miss5){
		...
		WriteHTMLUtil.writeHtm("/js/analyse/"+getLottery().getKey()+"/","xjel11to5_max_miss.js", "xjel11to5_max_miss="+root+";", "UTF-8");
	}
