...
@Service("klpkMissDataControllerServiceImpl")
public class KlpkMissDataControllerServiceImpl extends
		MissDataControllerServiceImpl<KlpkMissDataInfo, KlpkIssueData> {
	@Autowired
	private KlpkIssueDataDao issueDataDao;
	@Autowired
	private KlpkMissDataEntityManagerImpl missDataEntityManager;
	...
	public void createMissFiles() {
		...
		List<KlpkMissDataInfo> list = missDataEntityManager.getLastMissDatas(500);
		...
		// 创建遗漏文件
		for (KlpkMissDataInfo info : list) {
		...
		WriteHTMLUtil.writeHtm("/js/analyse/" + getLottery().getKey() + "/", "el11to5data.js", "dataEl11to5="
				+ resultRoot.toString() + ";function getdata(){return dataKlpk;}", "UTF-8");
	...
	private void createMaxMissFiles(JSONObject nummiss, JSONObject num1miss, JSONObject num2miss, JSONObject num3miss,
		...
		WriteHTMLUtil.writeHtm("/js/analyse/" + getLottery().getKey() + "/", "klpk_max_miss.js",
				"klpk_max_miss=" + root + ";", "UTF-8");
	}
	...
	@Override
	public MissDataEntityManagerImpl<KlpkMissDataInfo, KlpkIssueData> getMissDataManager() {
	...
	public void updateGroupNumMiss(int periodCount) {
		...
		List<KlpkIssueData> list = null;
		DetachedCriteria criteria = DetachedCriteria.forClass(KlpkIssueData.class);
