...
@Service("sdel11to5MissDataControllerServiceImpl")
public class SdEl11to5MissDataControllerServiceImpl extends
		MissDataControllerServiceImpl<SdEl11to5MissDataInfo, SdEl11to5IssueData> {
	@Autowired
	...
	public void createMissFiles() {
		...
		List<SdEl11to5MissDataInfo> list = missDataEntityManager.getLastMissDatas(500);
		...
		// 创建遗漏文件
		for (SdEl11to5MissDataInfo info : list) {
		...
		WriteHTMLUtil.writeHtm("/js/analyse/" + getLottery().getKey() + "/", "el11to5data.js", "dataEl11to5="
				+ resultRoot.toString() + ";function getdata(){return dataSdEl11to5;}", "UTF-8");
	...
	private void createMaxMissFiles(JSONObject nummiss, JSONObject num1miss, JSONObject num2miss, JSONObject num3miss,
		...
		WriteHTMLUtil.writeHtm("/js/analyse/" + getLottery().getKey() + "/", "sdel11to5_max_miss.js",
				"sdel11to5_max_miss=" + root + ";", "UTF-8");
	}
	...
	@Override
	public MissDataEntityManagerImpl<SdEl11to5MissDataInfo, SdEl11to5IssueData> getMissDataManager() {
	...
	public void updateGroupNumMiss(int periodCount) {
		...
		List<SdEl11to5IssueData> list = null;
		DetachedCriteria criteria = DetachedCriteria.forClass(SdEl11to5IssueData.class);
//		criteria.add(Restrictions.isNotNull("results"));
		...
		for (String contentKey : contentKeys) {
			...
			for (SdEl11to5IssueData data : list) {
