...
import com.cai310.lottery.common.miss.KenoMissDataUpdate;
...
public abstract class MissDataControllerServiceImpl<T extends MissDataInfo, P extends KenoPeriod> implements
		MissDataControllerService {

	protected Logger logger = LoggerFactory.getLogger(this.getClass());
	public abstract IssueDataDao<P> getIssueDataDao();
	
	public abstract MissDataEntityManagerImpl<T,P> getMissDataManager();
	...
	public void updateMissData() {
		...
		T missData = getMissDataManager().getLastMissData();
		if (missData != null) {
			criteria.add(Restrictions.gt("id", missData.getPeriodId()));
		} else {
			try {
		...
		List<KenoPeriod> list = getIssueDataDao().findByDetachedCriteria(criteria);
		if (list != null && !list.isEmpty()) {
			logger.info("开始进行遗漏数据更新!");
			List<MissDataFilter> filter = createFilters();
			try {
				KenoMissDataUpdate missUpdate = new KenoMissDataUpdate(list, missData, filter);
	...
	public void updateNumberAnalyse() {
		...
		List<P> list = getIssueDataDao().findByDetachedCriteria(criteria);
		if (list == null || list.isEmpty()) {
			logger.info("无开奖数据");
			return;
		}
		...
		for (P data : list) {
			...
			for (AnalyseFilter filter : analyseFilters) {
				filter.checkResult(data.getResults());
			}
		}
		for (AnalyseFilter filter : analyseFilters) {
		...
		KenoPeriod kenoPeriod = getIssueDataDao().get(lastPeriodId);
		if (ns == null) {
			ns = new NumberAnalyse();
			ns.setLottery(getLottery());
		}
		ns.setLastPeriodId(lastPeriodId);
		ns.setLastPeriodNumber(kenoPeriod.getPeriodNumber());
	...
	public void updateGroupNumMiss() {
		...
		for (P data : list) {
			lastPeriodId = data.getId();
			String[] arr = data.getResults().split(",");
			Arrays.sort(arr);
	 ...
	 */
	public void createRightFile(Map<String,Object> contents,String sourcefile,String destinationFileName,KenoPeriod kenoPeriod)throws DataException{
		try {
			TemplateGenerator tg=new TemplateGenerator(Constant.ROOTPATH+"/WEB-INF/content/"+kenoPeriod.getLotteryType().getKey());
			tg.create(sourcefile, contents, destinationFileName, Constant.ROOTPATH+"/WEB-INF/content/"+kenoPeriod.getLotteryType().getKey());
