...
@Service("gdel11to5MissDataControllerServiceImpl")
public class GdEl11to5MissDataControllerServiceImpl extends
		MissDataControllerServiceImpl<GdEl11to5MissDataInfo, GdEl11to5IssueData> {
	@Autowired
	...
	public void createMissFiles() {
		...
		List<GdEl11to5MissDataInfo> list = missDataEntityManager.getLastMissDatas(500);
		...
		// 创建遗漏文件
		for (GdEl11to5MissDataInfo info : list) {
	...
	private void createMaxMissFiles(JSONObject nummiss, JSONObject num1miss, JSONObject num2miss, JSONObject num3miss,
		...
		WriteHTMLUtil.writeHtm("/js/analyse/" + getLottery().getKey() + "/", "gdel11to5_max_miss.js",
				"gdel11to5_max_miss=" + root + ";", "UTF-8");
	}
	...
	@Override
	public MissDataEntityManagerImpl<GdEl11to5MissDataInfo, GdEl11to5IssueData> getMissDataManager() {
	...
	public void updateGroupNumMiss(int periodCount) {
		...
		List<GdEl11to5IssueData> list = null;
		DetachedCriteria criteria = DetachedCriteria.forClass(GdEl11to5IssueData.class);
//		criteria.add(Restrictions.isNotNull("results"));
		...
		for (String contentKey : contentKeys) {
			...
			for (GdEl11to5IssueData data : list) {
...
//						map.put("bp", Long.valueOf(map.get("ep") + ""));
	...
	private void createMaxMissFiles(JSONObject miss1,JSONObject miss2,JSONObject miss3,JSONObject miss4,JSONObject miss5){
		...
		WriteHTMLUtil.writeHtm("/js/analyse/"+getLottery().getKey()+"/","gdel11to5_max_miss.js", "gdel11to5_max_miss="+root+";", "UTF-8");
	}
