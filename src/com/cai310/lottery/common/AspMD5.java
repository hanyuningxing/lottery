...
public class AspMD5 {
	public static void main(String[] args) {
		AspMD5 aspMD5 = new AspMD5();
		System.out.println(aspMD5.calcMD5("D45P6adminDS1FEW"));
	}
	...
	private int[] str2blks_MD5(String str) {
		...
		java.nio.charset.Charset charset = Charset.forName("x-mswin-936");
		ByteBuffer byteBuffer = charset.encode(str);
		int[] gb_chars = new int[str.length()];
		int count = 0;
		int j = 0;
		while (j < byteBuffer.limit()) {
			int c = byteBuffer.get(j);
			if (c < 0) {
				c = byteBuffer.getShort(j);
				j += 2;
			} else {
				j++;
			}
	...
	public String calcMD5(String str) {
		...
		return rhex(b) + rhex(d) + rhex(c) + rhex(a);
	}
