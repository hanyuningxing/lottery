...
public class KenoMissDataUpdate {

	private List<KenoPeriod> results;
	 ...
	 */
	public KenoMissDataUpdate(List<KenoPeriod> results, MissDataInfo preInfo, List<MissDataFilter> filters)
			throws DataException {
	...
	public List<MissDataInfo> update() {
		...
		for (KenoPeriod kenoPeriod : results) {
			try {
				...
				newInfo.setPeriodId(kenoPeriod.getId());
				newInfo.setResult(kenoPeriod.getResults());
				for (MissDataFilter filter : filters) {
					if (filter.accect(kenoPeriod.getResults())) {// 命中
