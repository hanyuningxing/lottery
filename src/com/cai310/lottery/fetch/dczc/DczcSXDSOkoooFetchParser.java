...
public class DczcSXDSOkoooFetchParser extends DczcOkoooFetchParser {

	@Override
	public PlayType getPlayType() {
		return PlayType.SXDS;
	}
	...
	protected String getSourceXml() {
		return "http://buy.okooo.com/Upload/xml/46.xml";
	}
				...
				if (node instanceof LabelTag) {
					...
					for (Node node2 : nodeArr2) {
						if (node2 instanceof Span) {
							...
							if (OKOOO_ITEM_TRANSFORM.getOkoooItem(item).equals(span.getAttribute("name"))) {
								String rateStr = span.toPlainTextString().replaceAll("&nbsp;", "");
								Double rate = StringUtils.isNotBlank(rateStr) ? Double.valueOf(rateStr) : 0;
								itemMap.put(item.toString(), rate);
							}
						}
