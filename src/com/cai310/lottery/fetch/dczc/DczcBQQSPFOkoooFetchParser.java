...
public class DczcBQQSPFOkoooFetchParser extends DczcOkoooFetchParser {

	@Override
	public PlayType getPlayType() {
		return PlayType.BQQSPF;
	}
	...
	protected String getSourceXml() {
		return "http://buy.okooo.com/Upload/xml/48.xml";
	}
	...
	protected Map<String, Double> getItemMap(TableRow row) {
		...
		for (int i = 0, k = 7, len = itemArr.length; i < len; i++, k += 1) {
			Item item = itemArr[i];
	...
	private static final OkoooItemTransform OKOOO_ITEM_TRANSFORM = new SimpleOkoooItemTransform(
			new HashMap<Item, String>() {
				...
				{
					Item[] itemArr = ItemBQQSPF.values();
