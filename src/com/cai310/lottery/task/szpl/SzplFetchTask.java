package com.cai310.lottery.task.szpl;
...
public class SzplFetchTask extends BaseFetchTask{
	protected final transient Logger logger = LoggerFactory.getLogger(this.getClass());
	
	public void fetchDataByNum() {
		String content;
		try {
			content = HttpUtil.getContentByURL(FetchConstant.SZPL_NUM_URL, false);
	...
	public void fetchDataByPos() {
		String content;
		try {			
			content = HttpUtil.getContentByURL(FetchConstant.SZPL_POS_URL, false);
	...
	public void fetchDataByDZ() {
		String content;
		try {			
			this.logger.info("[{}]数字排列线程开始执行,组选三 组选六...");
			//组选三
			content = HttpUtil.getContentByURL(FetchConstant.SZPL_GROUP_DZ_URL_2, false);
			DataModel data = fiterContentByDZ(content,"2","1","1");
			BuildXMLDocByDZ(data);		
			//组选六
			content = HttpUtil.getContentByURL(FetchConstant.SZPL_GROUP_DZ_URL_3, false);
		...
		this.logger.info("[{}]数字排列线程开始执行结束,组选三 组选六...");
	}	
	...
	public void fetchDataByDG() {
		...
		try {						
			//组选三
			content = HttpUtil.getContentByURL(FetchConstant.SZPL_GROUP_DG_URL_2, false);
			DataModel data = fiterContentByDG(content,"1","1","1");
			BuildXMLDocByDG(data);	
			//组选六
			content = HttpUtil.getContentByURL(FetchConstant.SZPL_GROUP_DG_URL_3, false);
	...
	public void BuildXMLDoc(DataModel data) throws IOException, JDOMException {
        ...
        // 给父节点root添加items子节点;    
	       ...
	       // 输出xml文件(文件名：当前期)玩法；	       
		   XMLOut.output(Doc, new FileOutputStream(new File(Constant.ROOTPATH+FetchConstant.SZPL_NUM_PATH_1+data.getPeriod()+".xml")));
	...
	public static void BuildXMLDocByPos(DataModel data) throws IOException, JDOMException {
		   ...
		   XMLOut.output(Doc, new FileOutputStream(new File(Constant.ROOTPATH+FetchConstant.SZPL_NUM_PATH_2+data.getPeriod()+".xml")));
	  }	
	...
	public static void BuildXMLDocByDZ(DataModel data) throws IOException, JDOMException {
	       ...
	       XMLOut.output(Doc, new FileOutputStream(new File(Constant.ROOTPATH+FetchConstant.SZPL_GROUP_PATH_1+data.getPeriod()+"_"+data.getGroupType()+".xml")));
	    }	
	...
	public static void BuildXMLDocByDG(DataModel data) throws IOException, JDOMException {
	       ...
	       //注意 按统计方式 fxtype=1是按单个号码 fxtype=2是按单注号码 
	       XMLOut.output(Doc, new FileOutputStream(new File(Constant.ROOTPATH+FetchConstant.SZPL_GROUP_PATH_2+data.getPeriod()+"_"+data.getGroupType()+".xml")));
}	
