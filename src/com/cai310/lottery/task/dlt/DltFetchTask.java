package com.cai310.lottery.task.dlt;
...
public class DltFetchTask extends BaseFetchTask{
	protected final transient Logger logger = LoggerFactory.getLogger(this.getClass());
	
	public void fetchData() {
		String content;
		try {
			content = HttpUtil.getContentByURL(FetchConstant.DLT_CUR_URL, false);
			DataModel data = fiterContent(content,"1","1");
			BuildXMLDoc(data);
			
			content = HttpUtil.getContentByURL(FetchConstant.DLT_CUR_SORT_XTYPE_1, false);
	...
	public void BuildXMLDoc(DataModel data) throws IOException, JDOMException {
       ...
       //创建节点 items;
	    ...
	    if("1".equals(data.getPlayType())){
	    	XMLOut.output(Doc, new FileOutputStream(new File(Constant.ROOTPATH+FetchConstant.DLT_CUR_PATH_1+data.getPeriod()+".xml")));
	    }else{
	    	XMLOut.output(Doc, new FileOutputStream(new File(Constant.ROOTPATH+FetchConstant.DLT_CUR_PATH_2+data.getPeriod()+".xml")));
	    }
