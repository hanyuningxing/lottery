...
public class LczcSchemeCreateForm extends SchemeCreateForm {

	/** 选择的场次内容 */
	private LczcCompoundItem[] items = new LczcCompoundItem[12];

	/** 单式内容 */
	private String content;

	public LczcSchemeCreateForm() {
		super();
		for (int i = 0; i < items.length; i++) {
			items[i] = new LczcCompoundItem();
		}
	}
	...
	protected ContentBean buildCompoundContentBean() throws DataException {
		...
		if(selectedMatchCount<ZcUtils.LCZC_MATCH_COUNT * 2){
	...
	public LczcCompoundItem[] getItems() {
		return items;
	}

	public void setItems(LczcCompoundItem[] items) {
		this.items = items;
	}
