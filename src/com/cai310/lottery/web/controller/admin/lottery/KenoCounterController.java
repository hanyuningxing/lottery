...
@Result(name="success",location="/WEB-INF/content/admin/lottery/keno-counter.ftl")
public abstract class KenoCounterController<S extends NumberScheme> extends AdminBaseController{
	...
	public abstract DetachedCriteria createCountCriteria();
	...
	public String count(){
		...
		for(S scheme : schemeList){
			...
			if(scheme.getSchemeCost() != null){
				counter.setTotalSales(counter.getTotalSales().add(new BigDecimal(scheme.getSchemeCost())));
			}
			
			if(scheme.getSchemeCost() != null){
				counterAll.setTotalSales(counterAll.getTotalSales().add(new BigDecimal(scheme.getSchemeCost())));
			}
