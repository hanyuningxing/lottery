...
public class CommonUtil {
	...
	public static double roundPrize(double prize) {
		BigDecimal decimal = BigDecimal.valueOf(prize);
	 ...
	 * @return 银行家算法
	 */
	public static double bankRoundPrize(double prize) {
		BigDecimal decimal = BigDecimal.valueOf(prize);
		decimal = decimal.setScale(6, BigDecimal.ROUND_DOWN);// 保留六位
		decimal = decimal.add(BigDecimal.valueOf(0.000001));//去除double的计算失误
		decimal = decimal.setScale(3, BigDecimal.ROUND_DOWN);// 保留三位小数，后面的舍去
		decimal = decimal.setScale(2, BigDecimal.ROUND_HALF_EVEN);// 
		return decimal.doubleValue();
	}
	
	public static double floorPrize(double prize){
		BigDecimal decimal = BigDecimal.valueOf(prize);
		DecimalFormat formater = new DecimalFormat();
        formater.setMaximumFractionDigits(2);
        formater.setGroupingSize(0);
        formater.setRoundingMode(RoundingMode.FLOOR);
        return Double.valueOf(formater.format(decimal));
	}
}
