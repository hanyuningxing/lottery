...
public enum SscPlayType {
	 ...
	 **/
	RANDOMONE("任选1",Integer.valueOf("11"),1) {
		@Override
		public SscWinUnits calcWinUnit(Integer[] results, SscCompoundContent compoundContent, int multiple) {
			int resultNum5 = results[4];// 个位
			int resultNum4 = results[3];// 十位
			...
			List<Integer> bets = NumUtils.toIntegerList(compoundContent.getArea5List());
			if(bets.contains(resultNum5)) {
				sscWinUnits.addRandom1WinUnits(multiple);
			}
			bets = NumUtils.toIntegerList(compoundContent.getArea4List());
			if(bets.contains(resultNum4)) {
				sscWinUnits.addRandom1WinUnits(multiple);
			}
			bets = NumUtils.toIntegerList(compoundContent.getArea3List());
			if(bets.contains(resultNum3)) {
				sscWinUnits.addRandom1WinUnits(multiple);
			}
			bets = NumUtils.toIntegerList(compoundContent.getArea2List());
			if(bets.contains(resultNum2)) {
				sscWinUnits.addRandom1WinUnits(multiple);
			}
			bets = NumUtils.toIntegerList(compoundContent.getArea1List());
			if(bets.contains(resultNum1)) {
				sscWinUnits.addRandom1WinUnits(multiple);
			}
			return sscWinUnits;
		}
		...
		public Integer countUnit(SscCompoundContent compoundContent) {
			Integer units = 0;
			if(null!=compoundContent.getArea5List()&&compoundContent.getArea5List().size()>0){
				units += compoundContent.getArea5List().size();
			}
			if(null!=compoundContent.getArea4List()&&compoundContent.getArea4List().size()>0){
				units += compoundContent.getArea4List().size();
			}
			if(null!=compoundContent.getArea3List()&&compoundContent.getArea3List().size()>0){
				units += compoundContent.getArea3List().size();
			}
			if(null!=compoundContent.getArea2List()&&compoundContent.getArea2List().size()>0){
				units += compoundContent.getArea2List().size();
			}
			if(null!=compoundContent.getArea1List()&&compoundContent.getArea1List().size()>0){
				units += compoundContent.getArea1List().size();
			}
			return 	units;
		}
	},
	 ...
	 **/
	RANDOMTWO("任选2",Integer.valueOf("116"),1) {
		@Override
		public SscWinUnits calcWinUnit(Integer[] results, SscCompoundContent compoundContent, int multiple) {
			int resultNum5 = results[4];// 个位
			int resultNum4 = results[3];// 十位
			...
			sscWinUnits.addRandom2WinUnits(checkWin(0, hits, 0, 5, 2, 2) * multiple);
			return sscWinUnits;
		}
		@Override
		public Integer countDanUnit(List<String> danbets, List<String> tuobets) {
		...
		public Integer countUnit(SscCompoundContent compoundContent) {
			Integer units = 0;
			final List<Random2Item> betList = new ArrayList<Random2Item>();
			Random2Item random2Item = null;
			if(null!=compoundContent.getArea5List()&&compoundContent.getArea5List().size()>0){
				random2Item = new Random2Item();
				random2Item.setSelectedNum(compoundContent.getArea5List().size());
				betList.add(random2Item);
			}
			if(null!=compoundContent.getArea4List()&&compoundContent.getArea4List().size()>0){
				random2Item = new Random2Item();
				random2Item.setSelectedNum(compoundContent.getArea4List().size());
				betList.add(random2Item);
			}
			if(null!=compoundContent.getArea3List()&&compoundContent.getArea3List().size()>0){
				random2Item = new Random2Item();
				random2Item.setSelectedNum(compoundContent.getArea3List().size());
				betList.add(random2Item);
			}
			if(null!=compoundContent.getArea2List()&&compoundContent.getArea2List().size()>0){
				random2Item = new Random2Item();
				random2Item.setSelectedNum(compoundContent.getArea2List().size());
				betList.add(random2Item);
			}
			if(null!=compoundContent.getArea1List()&&compoundContent.getArea1List().size()>0){
				random2Item = new Random2Item();
				random2Item.setSelectedNum(compoundContent.getArea1List().size());
				betList.add(random2Item);
			}
			return  UnitsCountUtils.countUnits(2, new ArrayList<Random2Item>(), -1, -1, betList);
		}
	},
	/** 四星直选
	 * 从四位各选1个或多个号码，选号与开奖号码后四位按位一致即中奖1万元
	 **/
	DirectFour("四星直选",Integer.valueOf("10000"),4) {
		@Override
		public SscWinUnits calcWinUnit(Integer[] results, SscCompoundContent compoundContent, int multiple) {
			SscWinUnits sscWinUnits = new SscWinUnits();
			int resultNum5 = results[4];// 个位
			...
			if (area5.contains(resultNum5) && area4.contains(resultNum4) && area3.contains(resultNum3)
					&& area2.contains(resultNum2)) {
				sscWinUnits.addDirectFourWinUnits(multiple);
			}
			return sscWinUnits;
		}
		...
		@Override
		public Integer countUnit(SscCompoundContent compoundContent) {
			return  compoundContent.getArea2List().size()
					* compoundContent.getArea3List().size() * compoundContent.getArea4List().size()
					* compoundContent.getArea5List().size();
		}
