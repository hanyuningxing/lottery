...
public class SscWinUnits {
	...
	/** 五星直选  中奖注数 从0000~9999中选一个五位数投注，奖金10000元！ **/
	private Integer directFourWinUnits;
	...
	private Integer  random1WinUnits;
	
	private Integer  random2WinUnits;
	/**
	...
	public void reset() {
		...
		//任选1任选2
		this.setRandom1WinUnits(null);
		this.setRandom2WinUnits(null);
		//四星
		this.setDirectFourWinUnits(null);
	}
	...
	private Integer[] getWinUnits() {
		return new Integer[] { 
				...
				//任选1任选2
				this.getRandom1WinUnits(),
				this.getRandom2WinUnits(),
				//四星
				this.getDirectFourWinUnits()
		};
	}
	...
	public List<WinItem> getWinItemList() {
		...
		//任选1任选2
		unit = this.getRandom1WinUnits();
		if (unit != null && unit > 0) {
			list.add(new WinItem(SscPlayType.RANDOMONE.getTypeName(), unit));
		}
		unit = this.getRandom2WinUnits();
		if (unit != null && unit > 0) {
			list.add(new WinItem(SscPlayType.RANDOMTWO.getTypeName(), unit));
		}
		//四星
		unit = this.getDirectFourWinUnits();
		if (unit != null && unit > 0) {
			list.add(new WinItem(SscPlayType.DirectFour.getTypeName(), unit));
		}
		return list;
	}
	...
	public String getPrizeDetail() {
		...
		unit = this.getRandom1WinUnits();
		if (unit != null && unit > 0) {
			prize = SscPlayType.RANDOMONE.getPrize();
			varWonLineText.setVar("PRIZEITEM", SscPlayType.RANDOMONE.getTypeName());
			varWonLineText.setVar("WINUNITS", unit);
			varWonLineText.setVar("PRIZE", prize >= 10000.0d ? unit * prize * 0.8d : unit * prize);
			prizeDesc += varWonLineText.toString();
		}
		...
		unit = this.getRandom2WinUnits();
		if (unit != null && unit > 0) {
			prize = SscPlayType.RANDOMTWO.getPrize();
			varWonLineText.setVar("PRIZEITEM", SscPlayType.RANDOMTWO.getTypeName());
			varWonLineText.setVar("WINUNITS", unit);
			varWonLineText.setVar("PRIZE", prize >= 10000.0d ? unit * prize * 0.8d : unit * prize);
			prizeDesc += varWonLineText.toString();
		}
		...
		unit = this.getDirectFourWinUnits();
		if (unit != null && unit > 0) {
			prize = SscPlayType.DirectFour.getPrize();
			varWonLineText.setVar("PRIZEITEM", SscPlayType.DirectFour.getTypeName());
			varWonLineText.setVar("WINUNITS", unit);
			varWonLineText.setVar("PRIZE", prize >= 10000.0d ? unit * prize * 0.8d : unit * prize);
			prizeDesc += varWonLineText.toString();
		}
	...
	public void addDirectFourWinUnits(Integer winUnits) {
		if (this.directFourWinUnits != null) {
			this.directFourWinUnits += winUnits;
		} else {
			this.directFourWinUnits = winUnits;
		}
	}
	...
	public void addRandom1WinUnits(Integer winUnits) {
		if (this.random1WinUnits != null) {
			this.random1WinUnits += winUnits;
		} else {
			this.random1WinUnits = winUnits;
		}
	}
	public void addRandom2WinUnits(Integer winUnits) {
		if (this.random2WinUnits != null) {
			this.random2WinUnits += winUnits;
		} else {
			this.random2WinUnits = winUnits;
		}
	}
	...
	public void addSscWinUnits(SscWinUnits winUnit) {
		...
		this.addRandom1WinUnits(winUnit.getRandom1WinUnits());
		
		this.addRandom2WinUnits(winUnit.getRandom2WinUnits());
		
		this.addDirectFourWinUnits(winUnit.getDirectFourWinUnits());
	}
	...
	public Integer getRandom1WinUnits() {
		return random1WinUnits;
	}

	public void setRandom1WinUnits(Integer random1WinUnits) {
		this.random1WinUnits = random1WinUnits;
	}

	public Integer getRandom2WinUnits() {
		return random2WinUnits;
	}

	public void setRandom2WinUnits(Integer random2WinUnits) {
		this.random2WinUnits = random2WinUnits;
	}

	public Integer getDirectFourWinUnits() {
		return directFourWinUnits;
	}

	public void setDirectFourWinUnits(Integer directFourWinUnits) {
		this.directFourWinUnits = directFourWinUnits;
	}
