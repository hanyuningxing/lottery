...
public class CombinationObj <T extends Object>{
	/** 命中的胆码场次 */
	 ...
	 */
	public CombinationObj(int combinNum, List<T> danCorrectList, List<T> unDanCorrectList, int danMinHit,
			int danMaxHit) {
	...
	private void init() {
		ConverWorkExtensionCombCallBack call = new ConverWorkExtensionCombCallBack();
	...
	class ConverWorkExtensionCombCallBack implements ExtensionCombCallBack {
	...
	public static void main(String[] args) {
		List<String> danstrs = Lists.newArrayList();
		danstrs.add("A");
		danstrs.add("B");
		...
		strs.add("b");
		strs.add("c");
		CombinationObj objc = new CombinationObj(3,danstrs,strs,1,2);
		List<List<String>> res = objc.getResultList();
