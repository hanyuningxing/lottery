...
public class PrizeForecastItem implements Serializable {
	...
	private final List<JclqExtraMatchItem> danlist;
	private final List<JclqExtraMatchItem> undanlist;

	public PrizeForecastItem(Integer multiple, final List<JclqExtraMatchItem> list, Integer danMinHit,
			Integer danMaxHit, final List<Integer> passMatchList, final boolean min) {
		super();
		this.matchSize = list.size();

		this.danlist = new ArrayList<JclqExtraMatchItem>();
		this.undanlist = new ArrayList<JclqExtraMatchItem>();
		for (JclqExtraMatchItem item : list) {
			if (item.isDan())
	...
	private class ExtendCombCallBack implements ExtensionCombCallBack {
		private List<List<JclqExtraMatchItem>> detailList = new ArrayList<List<JclqExtraMatchItem>>();
		...
		public boolean run(boolean[] comb1, int m1, boolean[] comb2, int m2) {
			int n = m1 + m2;
			List<JclqExtraMatchItem> combList = new ArrayList<JclqExtraMatchItem>();
			double linePrize = 1;
			StringBuilder line = new StringBuilder();
			String passText = JclqConstant.PASS_TEXT[n - 1];
			line.append('[').append(passText).append(']');
			for (int i = 0; i < comb1.length; i++) {
				if (comb1[i]) {
					JclqExtraMatchItem item = danlist.get(i);
					combList.add(item);

					Double sp;
					if (item.isCancel()) {
						sp = item.selectedCount() * JclqConstant.CANCEL_MATCH_RESULT_SP;
						StringBuilder cancelSpText = new StringBuilder();
						for (int j = 0; j < item.selectedCount(); j++) {
							cancelSpText.append(CANCEL_SP_FORMAT.format(JclqConstant.CANCEL_MATCH_RESULT_SP)).append('+');
						}
			...
			for (int i = 0; i < comb2.length; i++) {
				if (comb2[i]) {
					JclqExtraMatchItem item = undanlist.get(i);
					combList.add(item);
			...
			linePrize*=UNITS_MONEY;
			linePrize = CommonUtil.bankRoundPrize(linePrize);
