...
public final class JclqUtil {
	 ...
	 */
	public static List<List<JclqMatchItem>> spliteMatchOfPlayType(int passMatchCount,List<JclqMatchItem> danList,List<JclqMatchItem> unDanList,Map<String,List<JclqMatchItem>> matchItemMap, Integer danMinHit, Integer danMaxHit){
		if(danMinHit==null)danMinHit=-1;
		if(danMaxHit==null)danMaxHit=-1;
		//按场次纵向过关分解
		JclqSchemeConverWork<JclqMatchItem> work = new JclqSchemeConverWork<JclqMatchItem>(passMatchCount, danList, unDanList, danMinHit, danMaxHit);
		List<List<JclqMatchItem>> combList = work.getResultList();
		List<List<JclqMatchItem>> selectedMatchItemsList = Lists.newArrayList();
		List<JclqMatchItem> selectedMatchItems = null;
		String matchkey = null;
		List<JclqMatchItem> matchItems = null;
		for(List<JclqMatchItem> combItems:combList){
			...
			for (Object[] objArrays : objectsArrays) {
				...
				for(Object item: objArrays){
					JclqMatchItem matchItem = (JclqMatchItem)item;
