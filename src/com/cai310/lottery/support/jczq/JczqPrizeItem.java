...
public class JczqPrizeItem implements Serializable {
	...
	/** 开奖结果 */
	private final Map<String, JczqResult> resultMap;

	/** 命中的场次组合 */
	private final List<JczqMatchItem> correctCombList;
	 ...
	 */
	public JczqPrizeItem(Map<String, JczqResult> resultMap, List<JczqMatchItem> correctCombList,Integer multiple) {
		super();
	...
	private void build() {
		String passText = JczqConstant.PASS_TEXT[correctCombList.size() - 1];
		...
		for (JczqMatchItem item : correctCombList) {
			JczqResult result = resultMap.get(item.getMatchKey());
			...
			if (result.isCancel()) {
				...
				spValue = selectedCount * JczqConstant.CANCEL_MATCH_RESULT_SP;

				StringBuilder spText = new StringBuilder();
				for (int i = 0; i < selectedCount; i++) {
					spText.append(CANCEL_SP_FORMAT.format(JczqConstant.CANCEL_MATCH_RESULT_SP)).append('+');
				}
