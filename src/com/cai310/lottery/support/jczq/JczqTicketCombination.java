...
public class JczqTicketCombination implements Serializable {
	 ...
	 */
	public JczqTicketCombination(PassMode passMode,PlayType playType, List<JczqMatchItem> matchItemList, PassType passType,
	...
	private void forecast() {
		...
		for (JczqMatchItem matchItem : this.matchItemList) {
			...
			if(PassMode.MIX_PASS.equals(this.passMode)){
				this.playType = matchItem.getPlayType();
			}
		...
		if (this.passType == PassType.P1) {
			forecastMinPrize = CommonUtil.bankRoundPrize(minList.get(0)) * this.multiple*2;
			forecastMaxPrize = 0.0;
			for (Double award : maxList) {
				forecastMaxPrize += CommonUtil.bankRoundPrize(award);
			}
			forecastMaxPrize *= this.multiple*2;
		} else {
			...
			forecastMinPrize *= 2d;
			forecastMinPrize = CommonUtil.bankRoundPrize(forecastMinPrize);
			...
			CombCallBack call = new CombCallBack() {
				...
				public boolean callback(boolean[] comb, int m) {
					...
					award *= 2d;
					award = CommonUtil.bankRoundPrize(award);
