...
public class CombinationBean implements Serializable {
	...
	public int countUnits() {
		final List<JczqMatchItem> danItems = new ArrayList<JczqMatchItem>();
		final List<JczqMatchItem> unDanItems = new ArrayList<JczqMatchItem>();
	...
	public <X extends Item> String toText(X[] allItems) {
		...
		for (JczqMatchItem matchItem : items) {
			sb.append(matchItem.getMatchKey()).append("→(");
	...
	public String generateKey() {
		...
		for (JczqMatchItem item : items) {
			sb.append(item.getMatchKey()).append(seq).append(item.getValue()).append(itemSeq);
		}
		sb.deleteCharAt(sb.length() - 1);
		return sb.toString();
	}
