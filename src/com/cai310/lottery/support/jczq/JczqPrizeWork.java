...
public abstract class JczqPrizeWork implements PrizeWork {
	/** 各个中奖组合中奖信息的分隔符 */
	...
	protected void handle(PassType passType, List<JcWonMatchItem> combList) {
		...
		List<JcWonMatchItem> cancelList = Lists.newArrayList();
	...
	protected void buildTicketWon() {
		if (!wonMap.isEmpty()) {
				...
				printWonItem.setIndex(0);
				printWonItem.setTotalPrize(BigDecimal.valueOf(totalPrize).multiply(BigDecimal.valueOf(this.multiple)).doubleValue());
				printWonItem.setTotalPrizeAfterTax(BigDecimal.valueOf(totalPrizeAfterTax).multiply(BigDecimal.valueOf(this.multiple)).doubleValue());
	...
	protected void buildSum() {
		...
		this.totalPrize =BigDecimal.valueOf(this.totalPrize).multiply(BigDecimal.valueOf(this.multiple)).doubleValue();
		this.totalPrizeAfterTax =BigDecimal.valueOf(this.totalPrizeAfterTax).multiply(BigDecimal.valueOf(this.multiple)).doubleValue();
