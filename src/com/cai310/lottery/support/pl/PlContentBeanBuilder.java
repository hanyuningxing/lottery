...
public class PlContentBeanBuilder {
	...
	public static ContentBean buildCompoundContentBean(String schemeContent, PlPlayType plPlayType)
		...
		List<String> reBallList = new ArrayList<String>();
		for (String str : arr) {
			// 重置
			reBallList.clear();
			area1List.clear();
			...
			if (plPlayType.equals(PlPlayType.P5Direct)) {
				...
				checkBall(area1List,0,9,true);
				checkBall(area2List,0,9,true);
				checkBall(area3List,0,9,true);
				checkBall(area4List,0,9,true);
				checkBall(area5List,0,9,true);
				...
				if (units != countUnits)
					throw new DataException("注数与系统计算的不一致.");
				checkBall(group3List,0,9,false);
				...
				checkBall(directSumList,0,27,true);
				for (String num : directSumList) {
					num = num.trim();
					int number = Integer.parseInt(num);
					if (number < 1 || number > 26)
						throw new DataException("方案内容不正确,号码在[01-26]之间.");
//					if(directSumList.size()==1){
//						//单独不能选
//						if(number==0||number==27)throw new DataException("方案内容不正确,不能只单选0或只单选27进行投注.");
//					}
				...
				for (String num : groupSumList) {
					...
					if (number < 1 || number > 26)
						throw new DataException("方案内容不正确,号码在[01-26]之间.");
//					if(groupSumList.size()==1){
//						//单独不能选
//						if(number==1||number==26)throw new DataException("方案内容不正确,不能只单选1或只单选26进行投注.");
//					}
					countUnits += PlConstant.UNITS_GROUP_SUM[number];
				}
				checkBall(groupSumList,0,26,true);
				if (units != countUnits)
					throw new DataException("注数与系统计算的不一致.");
				...
				checkBall(baoChuanList,0,9,true);
				...
				checkBall(directKuaduList,0,9,true);
				...
				checkBall(g3KuaduList,0,9,true);
				...
				checkBall(g6KuaduList,0,9,true);
	...
	public static void checkBall(List<String> area1List,int min,int max,boolean checekRepeat) throws DataException{
		List<String> reBallList = new ArrayList<String>();
		for (String num : area1List) {
			num = num.trim();
			int number = Integer.parseInt(num);
			if (number < min || number > max)
				throw new DataException("方案内容不正确,号码在["+min+"-"+max+"]之间.");
			if(checekRepeat){
				if (reBallList.contains(num) || reBallList.contains(num))
